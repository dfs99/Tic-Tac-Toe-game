...
# Board instance in order to access both the self.board and cells embedded in it
    ...
    for row in class_board_instance.board:
        for cell in row:
            if (cell.pos_x == pos_x) and (cell.pos_y == pos_y):
                if cell.button.GetName() == "player1":
                    # we don't have to place the token again
                    pass
                else:
                    # O
                    cell.button.SetLabel('O')
                    cell.button.SetName('player2')
                    cell.is_filled = True
...
class Board(wx.Panel):
    ...
    def set_token(self, event):
        ...
        global data_received
        if data_received[1] == "True":

            button = event.GetEventObject()
            if button.GetName() == '':
                # if the board is not full
                if self.is_board_full() is False:
                    button.SetLabel(TOKEN)
                    button.SetName('player1')
                    # mark the cell filled
                    for row in self.board:
                        for cell in row:
            ...
            # don't allow the player to place any token
            dialog = wx.MessageDialog(None,
